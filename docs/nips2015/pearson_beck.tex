...
\author{
John M.~Pearson\thanks{ Use footnote for providing further information
...
In vision, methods like reverse correlation have proven successful in reverse engineering response properties of some cell types, but such techniques rely on a well-behaved stimulus space and a highly constrained encoding model in order to achieve sufficient statistical power to perform inference \cite{steveninck1988realtime,ringach2004reverse,ringach2002receptive}. However, natural stimuli are known to violate both criteria, and generate patterns of neural activity that differ markedly from those observed in controlled experiments with limited stimulus complexity \cite{ringach2002receptive,sharpee2004analyzing,Vinje2000-dx}. 

work by Gallant lab and Sharpee paper on feature finding
    - downside: these are methods for linear readout?
    - how much feature space must be spelled out in advance?
    - Sharpee paper requires a metric on the stimulus space

more recently NLP methods:
    - fast
    - variational
    - have not focused as much on feature discovery

we sit at the intersection of the above two

figure 1:
    - graphical model diagram
    - cartoon of spikes/raster <---> firing matrix x feature matrix --> Poiss
