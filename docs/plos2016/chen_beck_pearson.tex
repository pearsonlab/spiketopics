...
\begin{figure}[!h]
	...
	\caption{\bf Illustration of repeated presentations of stimulus.} A: Original series of stimuli indexed by $t$. B: Examples of stimuli shown in two experiments and the corresponding spikes observed from neurons. Note that the number of exposure times $M_{tu}$ for each stimulus might be different.
	\label{fig:movie}
\end{figure}
...
\begin{equation}
    p(N, \Lambda, \theta) = p(N| \lambda, z, x, \theta)p(\Lambda|\lambda, z)
...
\end{equation}
with $\mathcal{H}$ the entropy. By adopting a similar trick of factorial HMM in \cite{ghahramani1997factorial}, we make a reasonable assumption that the posterior factorizes over each latent time series $z_{\bigcdot k}$ and the overdispersion factor $\theta_m$, as well as the rate parameters $\lambda_{\bigcdot u \bigcdot}$ associated with each Markov process. This factorization results in a variational posterior of the form:
\begin{multline}
...
\begin{figure}[!h]
	...
	A: Indicators of actual categories represented in the stimulus presentation period. The categories are not independent of each other. B: Recovered binary features during the stimulus presentation period. Note that model features 6 -- 9 are unused and that feature 2 closely tracks the Out feature of the data, albeit delayed. Feature 2 and 3 closely track the pulses of neurons when the coherence level change in the experiments. C: Actual and predicted firing rates for the stimulus period. Note that the model infers stimulus categories from the data, including appropriate timing of differentiation between categories.
	\label{roitman}
\end{figure}

\subsection*{Visual category data}
As a second test of our model, we applied our algorithm to a dataset comprising $U = 56$ neurons from macaque inferotemporal cortex \cite{McMahon2014-qq}. These neurons were repeatedly presented with 96 stimuli comprising 8 categories of monkey faces, monkey bodies, whole monkeys, natural scenes, food, manmade objects, and patterns (Figure \ref{imgclust}.A) in 1483 trials, rendering each stimulus expose to the neurons between 12 to 19 times. Data consisted of spike time series, which we binned into a 300ms pre-stimulus baseline, a $\sim$300ms stimulus presentation period, and a 300ms post-stimulus period. Three trials were excluded because of the abnormal stimulus presentation period ($>900$ms) which might result in a highly biased inference. To maximize interpretability of the results, we placed strong priors on the $\pi_k$ to encode the assumption that all features were off during the baseline period. We also modeled overdispersion with extremely weak priors to encourage the model to attribute fluctuations in firing to noise in preference to feature detection. We again fit $K = 10$ features with sparse hierarchical priors on population responses.

The inferred categories based on binned population responses are shown in Figure \ref{fig:imgclust}.B. Here we only show the major multiplicative effect in the stimulus presentation period that are larger than 5\%, by blacking out the multipliers that are close to 1 as they do not contribute much to the overall firing rate and might be heavily mixed with noises in spiking counts. Out from the original categories, our model successfully recovered the whole category of monkey faces by Feature 2, 4 and 5, with a total of more than 60\% increase in the firing rates. It also identified most stimulus in whole monkeys from Feature 2 and 4 with a total increase of 30\%, and some stimulus in the category of monkey body parts in Feature 4 at an increase of 8\%. The change in firing rates in the remaining categories are not apparent, thus leaving the other features close to a multiplicative of 1. This is consistent with the actual observed data, that neurons from macaque inferotemporal cortex is sensitive to stimuli that contain images of the same species. (need reference here?)

Note that the multiplicative effect in each feature is constant across all stimuli and neurons. This is because we assume that each feature in the model should represent and only represent a single effect on the whole set of neurons over all stimuli. Thus the total effect of a single stimulus is the sum of all features presented in that stimulus, e.g. the effect of Stimulus 1 in the monkey face category is the summation of effects presented by Feature 2, 4 and 5. In Figure \ref{fig:imgclust}.C and D we show the recovered firing rate of two neurons over all stimuli comparing to their original firing rate derived from data. The bars represent the 2.5 and 97.5 percentiles of firing rates from the data and simulated results, and the dots are the means of those firing rates regarding each stimulus. These two neurons are randomly picked up from the 56 and it is obvious that 1) the model successfully recovered the firing rates regarding each neuron to the same scale, and 2) the simulated results show smaller fluctuations in comparison to the actual data. 
...
\begin{figure}[!h]
	...
	A: Actual categories in the macaque data set. 96 stimuli comprising 8 categories were presented in 1483 trials, leaving each stimulus exposed to the neurons $\sim$15 times. B: The inferred states from our model. The color represents the multiplicative effect to the baseline firing rates. Note that our model shows a clear increase of firing rates in the categories of monkey faces, whole monkeys, and some stimuli within monkey bodies. C: Actual and predicted spikes per second across all stimulus of neuron 089a. D. Actual and predicted spikes per second across all stimulus of neuron 100a.
	\label{fig:imgclust}
\end{figure}
...
Moreover, without pre-assumed information, our model is even able to recover features that are neglected by the experimenters. When applying our model to the visual category data, we noticed that some features corresponding to specific stimuli would never show up or always show up no matter how we change the time bins or prior conditions. By a closer look into the recovered features, we found that some of the smaller features, although they do not contribute much to the overall change in firing rates, might be useful in identifying smaller differences in stimuli within the category. For example, we identified two subgroups in monkey faces that have the same presentation of features within the group, the first subgroup picked out all three monkey faces looking directly at the camera and the second subgroup picked out 4 monkey faces out of 5 that are looking to their left. 
