...
An additional feature of our approach is that the generated labels provide a concise and fairly complete summary of the stimulus related activity of all neural recordings, which can be observed by comparing the categorization performance of decoded neural activity to the categorization performance of the decoded features. Although we would like to emphasize that our model is not a data compression method, it does very well in preserving the majority information from the raw data by a relatively smaller dimensions (6 vs 10 features for the Roitman dataset and 10 vs 56 for the visual category data). As indicated by the inferred categories from Figure \ref{fig:imgclust}, when using a multinomial logistic regression with interaction terms to predict the categories of observed stimulus, our inferred features tied up with the raw data with 2-way interactions and outperforms with 3-way interactions (see Supplement), indicating that very little information in the neural recordings about the underlying was lost in this transformation.
