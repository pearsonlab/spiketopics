...
\textbf{S1 Text. \\}
\bigskip
\textbf{Mathematical derivation for ELBO and Inference} % Please use "title case" (capitalize all terms in the title except conjunctions, prepositions, and articles).
}
\newline
...
\end{align}
while the effective emission probabilities in State 1 are
\begin{align}
...
$\phi_{j u}$ as $j \in \{1, \ldots, \tau\}$  &$\sum_{\tau' \ge j} N_{\tau' u}$  &$\sum_{u, \tau \ge j} F_{t(\tau)} \prod_{\substack{\tau' \le \tau \\ \tau' \ne j}} \frac{\omega_{\tau' u}}{\zeta_{\tau' u}}$ \\
\end{tabular}
\end{center}
\end{table}
