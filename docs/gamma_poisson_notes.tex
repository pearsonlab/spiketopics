...
\subsubsection{Contributions to the evidence lower bound}
...
However, if we use the expanded state space $Z \otimes D$ with deterministic countdown nodes $D$, we can write the transitions as
\begin{align}
    p(z'|z, d) &=
        \begin{cases}
            A_{z'z}, & d=0 \\
...
\begin{equation}
    ...
    \right]
\end{equation}
with the boundary condition $d_T = 0$ Clearly,
\begin{equation}
    ...
    \end{cases}
\end{equation}
Clearly, the first condition is our constraint and the terms in the second condition do not contribute to the sum. For the third term, the correct weighting is via the two-slice marginals calculated above (which correctly handle the presence/absence of segment boundaries (i.e., $d=0$)):
\begin{equation}
    \mathbb{E}_q[\log p(z)] = \mathbb{E}_q\left[
    \sum_{t = 1}^{T - 1} \log p(z_{t + 1}, d_{t + 1}|z_t, d_t)
    \right] = \sum_{t = 1}^{T - 1}
    \mathrm{tr}(\Xi^T_{t+1, t} \cdot \log A) + \gamma_t^* \cdot \log p(d)
\end{equation}
where we have used 
\begin{align}
    \gamma_t(i) &= \sum_j \Xi_{t+1, t}(j, i) \\
    \gamma^*_t(j) &= \sum_i \Xi_{t+1, t}(j, i) 
\end{align}
and the dot products are sums over hidden state levels. Lastly, we can easily write
\begin{equation}
    \mathbb{E}_q[\log p(z_1, d_1)] = \xi_1 \cdot (\log \pi + \log p(d)) 
\end{equation}
