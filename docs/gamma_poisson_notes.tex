...
and the entropy:
\begin{align}
    H[q] &= \mathbb{E}_q[-\log q] = \mathbb{E}_q[p(\tau) + p(m|\tau)] \\
    &= \alpha - \log \beta + \log \Gamma(\alpha) + (1 - \alpha) \psi(\alpha) \\
    & - \frac{1}{2} \log \frac{\lambda}{2\pi} - \frac{1}{2} \left(\psi(\alpha) - \log \beta\right)
    + \frac{\lambda}{2} \left[
    \frac{1}{\lambda} + \mu^2 \left(1 - \frac{\alpha}{\beta}\right)
    \right]
\end{align}
Finally, we want to be able to calculate $\mathbb{E}_q[p(d|i)]$ in the normalization term above.
