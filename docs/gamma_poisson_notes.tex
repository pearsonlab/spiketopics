...
To calculate $Z \equiv p(y_{1:T})$, we need to sum all the segments that terminate at $T$ (the $F_T = 1$ condition) and all the segments that extend beyond $T$ ($F_T = 0$):
\begin{equation}
    Z = \sum_i \alpha_T(i) + \sum_{i, d + t > T} \alpha^*_t(i) \psi_{(t + 1):T}(i) p(d|i)
\end{equation}
Thus we can write:
\begin{align}
...
\begin{equation}
 ...
 \end{equation} 
 Moreover, we can easily calculate the normalization constant $W$ from the restriction that the sum of the variational posterior over $d$ is 1:
 \begin{equation}
     W_{ik} = \sum_{d} e^{\nu_{dik}}
 \end{equation}
...
\begin{equation}
    ...
    \frac{\beta^\alpha}{\hat{\beta}^{\alpha + \frac{1}{2}}}
\end{equation}
with 
\begin{equation}
    \hat{\beta} = \beta + \frac{1}{2} \frac{\lambda}{1 + \lambda} 
    (\log d - \mu)^2
\end{equation}
