...
\begin{align}
    ...
    &= \sum_t \alpha_t(i, d)\beta_t(i) \propto \gamma_t(i, d)
\end{align}
...
\begin{equation}
    \sum_{k, i, d} C(k, d, i) \left[ \log h(d) + 
    \mathbb{E}_q[\eta_{ki}] \cdot T(d) 
    - \mathbb{E}_q[A(\eta_{ki})] \right]
\end{equation}
with $k$ labeling hidden states, $i$ labeling levels of those states, and $\eta_{ki}$ the relevant parameters for the duration distributions.\footnote{Not to be confused with $\eta_{tk}$, the parameters of $q(z)$ corresponding to emission probabilities.}
