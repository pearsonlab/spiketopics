...
Here we consider the correlation of firing rates in consecutive natural time slots. Different from the discussion above, we include the moment-by-moment fluctuation in the firing rate of each unit:
\begin{equation}
...
\begin{align}
    \mathbb{E}_{q}[\cdot] &= \mathbb{E}_{q} \left[\sum_{\tau u} \log\phi_{\tau u} (\sum_{\tau' \ge \tau}N_{\tau' u}) \right]
    + \mathbb{E}_{q}\left[\sum_{tu} N_{tu} \sum_k z_{tk} \log\lambda_{ku} \right] \\
    &= \sum_{\tau u}\overline{\log\phi_{\tau u}} \sum_{\tau' \ge \tau}N_{\tau' u} + \sum_{ktu} N_{tu}\xi_{tk} \overline{\log \lambda_{ku}}
\end{align}
...
\begin{align*}
    \omega_{j u} &\leftarrow s_u + \sum_{\tau' \ge j} N_{\tau' u} \\
    \zeta_{j u} &\leftarrow r_u + \sum_{u, \tau \ge j} F_{t(\tau)} \prod_{\substack{\tau' \le \tau \\ \tau' \ne j}} \frac{\omega_{\tau' u}}{\zeta_{\tau' u}}
\end{align*}
