...
\begin{equation}
    ...
    = 0
\end{equation}
$\forall k < M, t$ which has the solution $\varepsilon = (\mathbf{1} \otimes \mathbf{1}) D$, with $D$ a diagonal matrix. That is, $\varepsilon$ is matrix with constant columns. Clearly, this is analogous with the case of $\tilde{\pi}$ and suggests that each column of $\tilde{A}$ can be normalized separately.

    {\bf To summarize:} Since none of the HMM parameters need to be properly normalized --- or more correctly, their normalizations can be absorbed into $Z$ --- it is feasible to treat all these as unconstrained free parameters.
