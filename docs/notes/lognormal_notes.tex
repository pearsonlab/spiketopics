...
\begin{equation}
    -\frac{1}{2}\sum_{uu'k}\mathbb{E}_q\left[\left(\Sigma_\varepsilon\right)^{-1}_{uu'}\right] \left[-2(\mu_\eta)_{tu'} c_{ku} + \sum_{k'}c_{ku}c_{k'u'}(\xi_{tk'} + (1 - \xi_{tk'})\delta_{kk'})\right]z_{tk}
\end{equation}
...
yielding
\begin{align}
    \log \psi_{tk} = \sum_u \tau_u \left[(\mu_\eta)_{tu} \cdot c_{ku} - \frac{1}{2} \sum_{k'} c_{ku} \cdot c_{k'u} (\xi_{tk'} + (1 - \xi_{tk'})\delta_{kk'})\right]
\end{align}
