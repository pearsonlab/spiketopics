...
\begin{align} 
    ...
    z_{1:T, k} &\sim \mathrm{HMM}(\varepsilon_k, \tilde{A}_k, \tilde{\pi}_k) \\
...
\end{equation}
where $Z \equiv p(N|\theta)$ and $\theta \equiv (\varepsilon, A, \pi)$. As we shall see, this posterior is straightforward to calculate via the forward-backward algorithm.
...
\begin{multline}
    ...
    \\
    - \sum_{kt} \left[ \xi_{tk}^T \varepsilon_{tk} + \mathrm{tr}\left(\Xi_{kt} \tilde{A}_k^T\right) + \xi_{0k}^T \tilde{\pi}_k - \log Z_{kt} \right]
\end{multline}
where we make use of the outputs of the forward-backward algorithm and the vector/matrix representations of $\tilde{A}$, $\tilde{\pi}$, $\varepsilon$, $\xi$, and $\Xi$. 

\section{Variational Updates}
Technically, the variational parameters above are $(\alpha, \beta, \gamma, \delta, \varepsilon, \tilde{A}, \tilde{\pi})$. However, $\xi = \xi(\varepsilon, \tilde{A}, \tilde{\pi})$ and $\Xi = \Xi(\varepsilon, \tilde{A}, \tilde{\pi})$ so we can instead treat these as the effective variational parameters. This then readily gives as updates:
\begin{align}
\varepsilon_{tk} &\leftarrow \sum_u N_{tu} \overline{\log \lambda_{ku}} + 
