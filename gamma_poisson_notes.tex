...
\subsection{Non-conjugate models}
We might not want to limit ourselves to $\mathrm{Ga}(\gamma_k, \gamma_k)$ priors in the above. In this case, we can still write down an optimization objective, which we must maximize by means that do not require closed-form solutions.

If we assume, (changing notation once again)
\begin{align}
    \lambda_{ku} &\sim \mathrm{Ga}(a_k, b_k) \\
    a_k &\sim \mathrm{Ga}(s_k, r_k) \\
    b_k &\sim \mathrm{Ga}(g_k, h_k) \\
    \lambda_{ku} &\simeq \mathrm{Ga}(\alpha_{ku}, \beta_{ku}) \\
    a_k &\simeq \mathrm{Ga}(\varsigma_{k}, \rho_{k}) \\
    b_k &\simeq \mathrm{Ga}(\gamma_{k}, \eta_{k})
\end{align}
with $\simeq$ denoting ``posterior approximation'', then we can write a lower bound to the optimization objective as
\begin{multline}
    \mathbb{E}_q\left[ 
    \frac{p(\lambda|a, b) p(a) p(b)}{q(\lambda) q(a) q(b)}
    \right] \ge
    \sum_k \bigg[
    \sum_u \left[
    (\overline{a_k} - 1) (\overline{\log \lambda_{ku}} + 1)
    - \overline{b_k} \overline{\lambda_{ku}} 
    \right] \\
    + U \overline{a_k} \left(\overline{\log b_k} - \overline{\log a_k} - \frac{1}{\varsigma_k}\right) + \frac{U}{2} \overline{\log a_k}  \\
    + (s_k - 1) \overline{\log a_k} - r_k \overline{a_k} 
    + (g_k - 1) \overline{\log b_k} - h_k \overline{b_k}
    \bigg] \\
    + \sum_{ku} H_g(\alpha_{ku}, \beta_{ku}) + \sum_k \left[H_g(\varsigma_k, \rho_k) + H_g(\gamma_k, \eta_k) \right]
\end{multline}
Where we have used
\begin{equation}
    X \sim \mathrm{Ga}(\alpha, \beta) \Rightarrow
    \mathbb{E}[X \log X] = \frac{\alpha}{\beta} (\psi(\alpha + 1) - \log \beta)
    = \frac{\alpha}{\beta} \left(\frac{1}{\alpha} + \psi(\alpha) - \log \beta\right)
\end{equation}
since
\begin{equation}
    \psi(x + 1) = \frac{d}{dx} \log \Gamma(x + 1) = \frac{d}{dx} \log x \Gamma(x) = \frac{1}{x} + \psi(x)
\end{equation}
and the barred quantities are trivial Gamma-family expectations.
