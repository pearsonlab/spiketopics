...
\begin{align}
    ...
    - \sum_{tu} \prod_k \left( 1 - \xi_{tk} + \xi_{tk} \frac{\alpha_{ku}}{\beta_{ku}}\right) &\rightarrow - \sum_{mu} \theta_{mu} \prod_k \left( 1 - \xi_{t(m)k} + \xi_{t(m)k} \frac{\alpha_{ku}}{\beta_{ku}}\right)
\end{align}
Note in particular that the overdispersion term is assumed to always be present, and that it is unique to every stimulus presentation ($m$ index), not merely every moment during the stimulus itself ($t$ index). 
...
\begin{multline}
    ...
    \sum_{u} M_u \left[ (s_u - 1) (\psi(\omega_u) - \log \eta_u) + r_\zeta \frac{\omega_u}{\zeta_u} + H_g(\omega_u, \zeta_u) \right]
\end{multline}
with $M_u \equiv \sum_t M_{tu}$ the total number of observations for unit $u$ across all time and $H_g$ the differential entropy for the gamma distribution, as above. By analogy with $\lambda$, we can easily write down the updates for $\omega$ and $zetaa$
\begin{align}
    \omega_u &\leftarrow \sum_m N_{mu} + s_u \\
