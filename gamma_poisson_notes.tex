...
Alternately, since we expect $\lvert (a/b) - 1 \rvert \ll 1$ when $x \gg 1$, we can write $b_{iu} = a_{iu}e^{-\epsilon_{iu}/\Lambda}$, with $\Lambda$ an arbitrary scaling constant, in which case
\begin{multline}
    \mathcal{L}_b = \sum_{iu} \left[a_{iu} \frac{\epsilon_{iu}}{\Lambda} - w_{iu} e^{\epsilon_{iu}/\Lambda} \right]\\
     - \sum_{mu} \frac{\omega_{mu}}{\zeta_{mu}} 
    \prod_k \left(1 - \xi_{t(m)k} + \xi_{t(m)k} 
    \frac{\alpha_{ku}}{\beta_{ku}}\right)
    e^{\sum_j \epsilon_{ju} x_{t(m)ju}/\Lambda}
\end{multline}
and
\begin{multline}
    \nabla_\epsilon \mathcal{L}_b = \frac{a_{iu}}{\Lambda} - \frac{w_{iu}}{\Lambda} e^{\epsilon_{iu}/\Lambda} \\
    - \frac{1}{\Lambda}\sum_m x_{t(m)iu} \frac{\omega_{mu}}{\zeta_{mu}} 
    \prod_k \left(1 - \xi_{t(m)k} + \xi_{t(m)k} 
    \frac{\alpha_{ku}}{\beta_{ku}}\right)
    e^{\sum_j \epsilon_{ju} x_{t(m)ju}/\Lambda} 
\end{multline}
