...
\begin{align} 
    ...
    z_{1:T, k} &\sim \mathrm{HMM}(\tilde{N}, \tilde{\lambda}_{k\bullet}, \tilde{A}_k, \tilde{\pi}_k) \\
...
\begin{align}
    ...
    \Xi_{kt, ij} \equiv p(z_{t+1,k} = j, z_{tk} = i|N, \theta) \qquad &\text{two-slice marginals}\\
...
\sum_{ik} \left[ (\nu_{ik1} - 1) \overline{\log A_{k1i}} + (\nu_{ik2} - 1) \overline{\log A_{k0i}} + H(A_{k1i}) \right]
\end{equation}
where 
\begin{align}
...
\begin{multline}
    ...
    \\
    - \sum_{kt} \left[ \xi_{tk}^T \varepsilon_{tk} + \mathrm{tr}\left(\Xi_{kt} \tilde{A_k^T}\right) + \xi_{0k}^T \tilde{\pi_k} - \log Z^{(k)}_t \right]
\end{multline}
where we make use of the outputs of the forward-backward algorithm and the vector/matrix representations of $\tilde{A}$, $\tilde{\pi}$, $\varepsilon$, $\xi$, and $\Xi$. Finally, gathering terms, we can write

\section{Variational Updates}
Because several of our assumed variational distributions are conjugate within the exponential family, we can immediately write down the optimal values of several variational paramters:
\begin{align}
foo    
\end{align}
